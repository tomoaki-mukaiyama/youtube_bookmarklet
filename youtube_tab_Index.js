...
javascript: (() => {
var e=document.createElement("style");e.type="text/css";e.innerText="a:focus{outline: none; border-style: solid; border-width: 7px; border-color: #E71D36;}";document.getElementsByTagName("HEAD").item(0).appendChild(e);document.getElementById("chips-wrapper").style.display="none";g();h();var k=document.querySelectorAll("ytd-rich-section-renderer");k&&k.forEach(function(a){a.style.display="none"});document.querySelector("#contents").querySelector("#thumbnail").focus();
function h(){if(!document.querySelectorAll("#primary")[0].classList.contains("eventListenerAdded")){document.querySelectorAll("#primary")[0].classList.add("eventListenerAdded");var a=0;document.querySelectorAll("#primary")[0].addEventListener("keydown",function(b){var d=document.querySelectorAll("#primary")[0].firstElementChild.getAttribute("style").split(":")[1].slice(0,1),c=document.querySelectorAll(".yt-simple-endpoint.inline-block.style-scope.ytd-thumbnail");c=Array.from(c);thumbnails=c.filter(function(f){return f.closest("ytd-rich-item-renderer.style-scope.ytd-rich-grid-renderer")});
c=thumbnails.find(function(f){return f===document.activeElement});c=thumbnails.indexOf(c);"w"===b.key?thumbnails[parseInt(a)-parseInt(d)]&&(a=parseInt(c)-parseInt(d),thumbnails[a].focus()):"s"===b.key?thumbnails[parseInt(a)+parseInt(d)]&&(a=parseInt(c)+parseInt(d),thumbnails[a].focus()):"a"===b.key?thumbnails[parseInt(a)-1]&&(a=parseInt(c)-1,thumbnails[a].focus()):"d"===b.key&&thumbnails[parseInt(a)+1]&&(a=parseInt(c)+1,thumbnails[a].focus())},!0)}}
function g(){var a=document.querySelectorAll(".yt-simple-endpoint.inline-block.style-scope.ytd-thumbnail");a=Array.from(a);thumbnails=a.filter(function(b){return b.closest("ytd-rich-item-renderer.style-scope.ytd-rich-grid-renderer")});thumbnails.forEach(function(b){"1"!==b.getAttribute("tabindex")&&(b.setAttribute("tabindex","1"),b.addEventListener("focus",function(){document.activeElement.scrollIntoView({behavior:"instant",block:"center"})}))})}var l=document.getElementById("contents");
(new MutationObserver(function(){g()})).observe(l,{childList:!0});
})()